...
class Vocab(object):
    ...
    def tag2id(self, xs):
        if isinstance(xs, list):
            return [self._tag2id.get(x,self.unktag) for x in xs]
        return self._tag2id.get(xs,self.unktag)
...
def creatVocab(corpusFile, min_occur_count):
    ...
    print("max seq length is : "+str(max_seq))

    return Vocab(word_counter, tag_counter, rel_counter, root, min_occur_count),max_seq
