...
def train(data, dev_data, test_data, parser, vocab, config):
    ...
    for iter in range(config.train_iters):
        ...
        for onebatch in data_iter(data, config.train_batch_size, True):
            words, extwords, tags, heads, rels, lengths, masks, positions = \
                batch_data_variable(onebatch, vocab)
            parser.model.train()

            parser.forward(words, extwords, tags, masks, positions)
...
def evaluate(data, parser, vocab, outputFile):
    ...
    for onebatch in data_iter(data, config.test_batch_size, False):
        words, extwords, tags, heads, rels, lengths, masks, positions = \
            batch_data_variable(onebatch, vocab)
        count = 0
        arcs_batch, rels_batch = parser.parse(words, extwords, tags, lengths, masks, positions)
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--thread', default=5, type=int, help='thread num')
